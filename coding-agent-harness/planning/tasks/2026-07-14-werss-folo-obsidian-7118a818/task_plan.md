...
12. 修复 Reader iframe 滚动链和整列渲染卡顿，加入渐进加载、正文预取、公众号栏折叠、全屏阅读和 Enter 快速批注。
13. 修复原生 annotator 的黄色默认值、越界整篇选区和拖选后弹窗误关闭，支持可留空的纯划线确认。
14. 将同步助手的微信消息正文按消息块增量导入 Reader，并以 Cloudflare Workflow、Access Service Token 和 Tunnel 取代本机/Codex 调度权。

## 验收标准

- [x] 公众号运营者授权和 12 个来源完成。
- [x] 当前 160 篇达到正文门槛的文章全部且唯一进入 Readeck，Readeck 容器原生 aarch64。
...
- [x] Reader 正文鼠标与键盘滚动、60 篇渐进加载、hover/focus 预取、公众号栏折叠、浏览器全屏和 Enter 快速批注经真实浏览器验证。
- [x] 真实鼠标拖选后划线预览与笔记框保持；空笔记 Enter 提交 `color:none` 纯划线；越界整篇误选被拒绝且不写数据。
...
- [x] 微信消息 140 条全部且唯一进入 Reader；无链接消息不再遗漏，重复运行新增 0。
- [x] Cloudflare Workflow、本机机器入口与双模式刷新控制端通过单测、dry-run 和本机 Caddy 端到端验证。
- [x] Reader Access Service Token/Service Auth policy、Workflow live deploy、一次 `sync`/`start` 成功实例与停用本机两个 Cron 完成。
- [ ] 72 小时/7 天近实时观察完成。
...
- Cloudflare Access Service Token 与 Service Auth policy 已用最小权限临时账户 Token 创建，临时 Token 已撤销；Worker 仅保留 Reader 专用机器凭据。
