...
本仓库不是 WeRSS 或 Readeck 源码 fork，而是本机部署与运维包。WeRSS 每小时抓取公众号；同步器把完整正文全量写入自托管 Readeck；用户在 Readeck 收藏、划线和批注；精选文章自动进入 SummerOS Archive，人工笔记和元数据永不覆盖。Cloudflare Access 负责一次性设备身份，Tunnel 只公开经过 JWT 校验的 Reader 入口，Folo/Tailscale 退出主链路。
...
| --- | --- | --- | --- | --- |
| `2026-07-14-werss-folo-obsidian-7118a818` | ARCH-001..013 | architecture SSoT | accepted; Folo 架构被 Readeck 架构取代 | 2026-07-15 |
