...
本仓库维护 WeRSS、只读 Caddy、Tailscale Funnel、Folo 和 SummerOS Obsidian 之间的公众号阅读链路。`coding-agent-harness/` 是唯一工程主账本。

## 项目概况

- 项目名：WeRSS + Folo + Obsidian 公众号阅读系统
- 仓库形态：单仓、本机部署包、外部 SaaS/桌面应用集成
- 运行环境：Apple Silicon macOS、Docker Compose、SQLite、Caddy、Tailscale
- 默认分支：`main`
...
3. WeRSS 管理端只允许绑定 `127.0.0.1:8001`，Caddy 只允许绑定 `127.0.0.1:8080`。
4. 公网只允许随机前缀下的 `GET/HEAD /feed/*.atom`；其他路径和方法必须 `404`。
5. WeRSS 与 Caddy 镜像使用固定摘要，不跟随 `latest`，不安装 Watchtower。
6. 固定 WeRSS 镜像只使用 WebKit；不要把 `BROWSER_TYPE` 改为 Firefox，除非先更换并验证镜像。
7. SQLite 备份前必须短暂停止 WeRSS；恢复演练只能在独立目录、不同 Compose project 和端口执行。
8. WeRSS 会在日志中打印环境变量；不得把原始容器日志写入公开 evidence、Issue 或文档。
9. `FEED_PREFIX` 是 bearer secret。Folo 导出的 `feedUrl` 可能包含它，公开笔记前必须删除。
10. Folo 只负责浏览、筛选和入库；永久批注和知识提升在 Obsidian 完成。
...
13. 资格、Docker 协议、微信扫码、Tailscale 首次批准、Folo 登录/付费和 72 小时观察属于人工门禁，不能虚报完成。
...
| Caddy、Tailscale、Folo、Obsidian 契约 | `coding-agent-harness/context/integrations/README.md` |
...
部署文件、脚本、Obsidian 结构和本机应用准备完成只是实现切片。只有资格、微信授权、Funnel、安全冒烟、Folo 72 小时/7 天观察、Obsidian 样本、备份恢复和对抗审查全部有证据后，整个任务才可完成。
