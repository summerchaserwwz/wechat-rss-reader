...
这样既不会把 100 多篇全文全部塞进 Obsidian，也不会把重要划线锁在某个付费云服务里。当前实测为 12 个公众号、108 篇 WeRSS 文章、104 篇达到正文门槛的 Readeck 完整文章。
...
正文采用 macOS 中文系统字体栈，桌面内容宽度控制在 680–760 px、行高约 1.8。移动端自动变成单栏逐级进入，顶部仍保留收藏、价值和主题入口：
...
打开文章本身不会误标已读。滚动达到正文 80% 后自动标为已读，也可以点击正文顶部“标为已读 / 标为未读”手工反转。
...
“订阅”Tab 会显示每个公众号的文章数、未读数、最近抓取和健康状态，并明确提示每小时第 17 分钟抓取、抓取后 5 分钟同步。

![Reader 订阅与抓取状态](images/12-reader-subscriptions.png)

新增公众号仍然只在部署 Mac 打开 `http://127.0.0.1:8001` 完成，公网不会得到 WeRSS 管理写接口。
