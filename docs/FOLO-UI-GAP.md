...
| 订阅追加 | 新增公众号必须由受保护的本机管理端完成 | Folo 来源栏直接显示订阅与未读计数 | “订阅”页展示来源、文章数、未读、最近抓取和健康；部署 Mac 可打开本机 WeRSS，公网不暴露管理端 | `reader-ui/`、`scripts/reading-sync.py` | 桌面显示本机链接；390px 手机只显示回到部署 Mac 的说明且不渲染链接；状态 JSON 已通过；N→N+1 仍为人工门禁 | 部分完成 |
...
- 边界：大面板统一 24px 圆角，文章卡统一 16px 圆角，操作按钮统一胶囊形；深度主要由近黑表面阶梯、细边框与顶部内高光形成。
- 字体：界面和嵌入正文优先使用 macOS 的 SF Pro 与苹方；正文改为无衬线、较高行距，并为引用、代码块、图片和文本选区提供一致样式。
- 语义色：靛蓝只用于当前选择与选区；橙色只用于未读、收藏和价值；主操作使用 Petdex 风格的高对比白色胶囊。
- 边界声明：只借鉴视觉语言，不复制 Petdex 的 Logo、角色卡片、文案或品牌素材。
